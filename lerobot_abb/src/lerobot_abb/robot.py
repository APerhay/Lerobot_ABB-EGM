...
class ABBRobot(Robot):
    ...
    def connect(self):
        ...
        self._thread.start()
        print(f"✅ ABB EGM Connected on port {self.config.port}")

    def _communication_loop(self):
        """High-speed EGM heartbeat loop."""
        try:
            while not self._stop_event.is_set():
                ...
                if success:
                    ...
                    # If we have a Koch arm action, send it; otherwise, stay at current pos
            ...
            print(f"EGM Loop Error: {e}")
        finally:
            if self.egm: self.egm.close()
